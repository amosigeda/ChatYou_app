...
public class VIPServiceImpl extends ICommServiceImpl implements VIPService {
	...
	@Override
	public MessageUtil getVIPSetMealList(int payType) {
		try {
			
			String sql = "SELECT t_id,t_setmeal_name,t_cost_price,t_money,t_gold,t_duration,t_set_meal_name FROM t_vip_setmeal WHERE t_is_enable = ? AND  t_pay_type = ? ";
			
			List<Map<String, Object>> vipList = this.getFinalDao().getIEntitySQLDAO().findBySQLTOMap(sql, VIPSetmeal.IS_ENABLE_NO,payType);
